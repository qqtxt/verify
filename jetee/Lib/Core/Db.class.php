...
class Db {
	...
	protected function initConnect($master=true) {
		if(1 == C('DB_DEPLOY_TYPE'))
			...
			// 默认单数据库
			if ( !$this->connected || IS_CLI) $this->_linkID = $this->connect();
	}
	 ...
	 * like分析  防注入
	 * @access protected
	 * @param mixed $value   %qq\_txt%   转成  '%qq_txt%'          %qq_txt%=>'%qq\_txt%'
	 * @return string
	 */
	protected function parseLike($value) {
		if(is_null($value)){
			$value =  '';
		}
		...
		return  '\''.$first.strtr($value, array('%' => '\%','\\' => '\\\\', '_' => '\_', '\_' => '_', "'" => "\'", '"' => '\"')).$end.'\'';
	}
	...
	/**
	 * where分析 不是复杂条件key安全过滤  不是中英文数字报错
	 * @access protected
	 * @param mixed $where
	 * @return string
	 */
	protected function parseWhere($where) {
		...
		if(is_string($where)) {
			...
			$operate  = isset($where['_logic'])?strtoupper($where['_logic']):'';  //array('user'=>'qqtxt','nickname'=>'qqtxt','_logic'=>'OR')  取操作符
			...
			foreach ($where as $key=>$val){//array(array('user_name'=>array('like','%aa%')),array('add_time'=>array('lg',2888888888)))
				$whereStr .= '( ';
				if(is_numeric($key)){
					$key  = '_complex';
				}
				if(0===strpos($key,'_')) {
					...
					// 查询字段的安全过滤
					if(!preg_match('/^[A-Z_\|\&\-.a-z0-9\(\)\,\x{4e00}-\x{9fa5}]+$/u',trim($key)))
						throw_exception(L('_EXPRESS_ERROR_').':'.$key);
					
					// 多条件支持
					$multi  = is_array($val) &&  isset($val['_multi']); //$map['status|title'] =array('1','thinkphp','_multi'=>true);
					$key    = trim($key);
					if(strpos($key,'|')) { // 支持 name|title|nickname 方式定义查询字段
						$array =  explode('|',$key);
						$str   =  array();
						foreach ($array as $m=>$k){
							$v =  $multi?$val[$m]:$val;//status '1' |status     array('1','thinkphp')
	...
	// where子单元分析   val 都用$this->escapeString 防注入
	protected function parseWhereItem($key,$val) { 
		$whereStr = '';
		if(is_array($val)) {
			if(is_string($val[0])) {
				$exp	=	strtolower($val[0]);
				if(preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT)$/i',$val[0])) { // 比较运算 'add_time' array('lg','2343984848')
					$whereStr .= $key.' '.$this->exp[$exp].' '.$this->parseValue($val[1]);
				}elseif(preg_match('/^(NOTLIKE|LIKE)$/i',$val[0])){// 模糊查找 
					if(is_array($val[1])) {								// 'user_name' array('like',array('%aa%','%bb%'),'or')
						$likeLogic  =   isset($val[2])?strtoupper($val[2]):'OR';
						if(in_array($likeLogic,array('AND','OR','XOR'))){
							$like       =   array();
							foreach ($val[1] as $item){
								$like[] = $key.' '.$this->exp[$exp].' '.$this->parseLike($item);
							}
							$whereStr .= '('.implode(' '.$likeLogic.' ',$like).')';         //(user_name like %aa% or user_name like %bb%)                 
						}
					}else{
						$whereStr .= $key.' '.$this->exp[$exp].' '.$this->parseLike($val[1]);
					}
				}elseif('exp'==$exp){ // 使用表达式  ['id']  = array('exp',' IN (1,3,8) ');   (id in (1,3,8))
					$whereStr .= ' ('.$key.' '.$val[1].') ';
				}elseif(preg_match('/^(NOTIN|NOT IN|IN)$/i',$exp)){ 
					if(isset($val[2]) && 'exp'==$val[2]) {//$map['id']  = array('in','exp','1,3,8');   (id in (1,3,8))
						$whereStr .= $key.' '.$this->exp[$exp].' '.$val[1];
					}else{
						if(is_string($val[1])) {  // IN 运算 ['id']  = array('in','1,3,8');     || ['id']  = array('in',array('1','3','8'));
							 $val[1] =  explode(',',$val[1]);
						}
			...
			}else { //['id'] = array(array('gt',3),array('lt',10), 'or') ;
				$count = count($val);
				...
				for($i=0;$i<$count;$i++) {
					...
					if('exp'==strtolower($val[$i][0])) {                //['id'] = array(array('exp','>3'),array('lt',10), 'or') ;     
						$whereStr .= '('.$key.' '.$data.') '.$rule.' ';  //(id  >3) or 
					}else{
						$op = is_array($val[$i])?$this->exp[strtolower($val[$i][0])]:'=';   
						$whereStr .= '('.$key.' '.$op.' '.$this->parseValue($data).') '.$rule.' ';  //(id <10) or  
					}
				}
	...
	/**
	 * join分析   array('__USER__ u on ','__USER_LOG__ l on ')  =>  LEFT JOIN je_user u on LEFT JOIN je_user_log l on
	 * @access protected
	 * @param mixed $join
	 * @return string
	 */
	protected function parseJoin($join) {
		...
		$joinStr = preg_replace_callback("/__([A-Z_-]+)__/sU",  function($matches) {return C("DB_PREFIX").strtolower($matches[1]);},   $joinStr);
		return $joinStr;
	}

	/**
	 * order分析  array('add_time'=>'desc','id'=>'asc')  array('add_time desc','id asc')
	 * @access protected
	 * @param mixed $order
	 * @return string
	 */
	...
	public function select($options=array()) {
		...
		if($cache) { // 查询缓存检测
			$key    =  'DBdata_'.(is_string($cache['key'])?$cache['key']:md5($sql));
			$value  =  json_decode(S($key,'',$cache));
			if(false !== $value) {
				return $value;
			}
		}
		...
		if($cache && false !== $result ) { // 查询缓存写入
			S($key,json_encode($result),$cache);
		}
		return $result;
	}
	...
	public function buildSelectSql($options=array()) {
		...
		if(C('DB_SQL_BUILD_CACHE')) { // SQL创建缓存
			$key    =  'sql_'.md5(serialize($options));
			$value  =  S($key);
			if(false !== $value) {
				return $value;
			}
