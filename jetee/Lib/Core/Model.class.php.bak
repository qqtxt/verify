...
class Model {
	...
	public function save($data='',$options=array()) {
		...
		#if(false === $this->_before_update($data,$options))			return false;
	...
	/**
	 * 分析表达式  主要是删除字段不在$this->fields的where字段  根据字段类型安全转换where intval floatval
	 * @access protected
	 * @param array $options 表达式参数
	 * @return array
	 */
	...
	/**
	 * 数据类型检测 根据类型安全转换类型 intval floatval
	 * @access protected
	 * @param mixed $data 数据
	...
	//与 getField一样
	public function getOne($field,$sepa=null) {
		return  $this->getField($field ? $field : $this->options['field'] ,$sepa);
	}
	...
	protected function returnResult($data,$type=''){
		if ($type){
			if(is_callable($type)){//检测参数是否为合法的可调用结构   如array($mysqliObject, 'ping');   或‘intval’
	...
	public function parseFieldsMap($data,$type=1) {
		// 检查字段映射
		if(!empty($this->_map)) {
			foreach ($this->_map as $key=>$val){//  array('name' =>'username','mail' =>'email',)
				if($type==1) { // 读取
					if(isset($data[$val])) {
						$data[$key] =   $data[$val];
						unset($data[$val]);
	 ...
	 */
	 public function create($data='',$type='') {
		...
		if(empty($data)) {
			...
			$data   =   get_object_vars($data);//把对象转为数组
		}
		// 验证数据
	...
	// 自动表单令牌验证 支持使用m("table")->token(false) 关闭令牌验证
	...
	public function where($where,$parse=null){
		if(!is_null($parse) && is_string($where)) {// 防止SQL注入
			if(!is_array($parse)) {
				$parse = func_get_args();
				array_shift($parse);
			}
		...
		}elseif(is_object($where)){					//转为数组
			$where  =   get_object_vars($where);
		}

		if(is_string($where) && '' != $where){//字符串只能一个  后面的换前面
			$map    =   array();
			$map['_string']   =   $where;
			$where  =   $map;
		}
