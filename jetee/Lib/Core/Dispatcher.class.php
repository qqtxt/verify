...
class Dispatcher {
	...
	static public function dispatch() { //先获取所有值 以后再判断安全
		if(isset($_GET[C('VAR_PATHINFO')])) { // 判断URL里面是否有兼容模式参数  nginx会把参数转为兼容模式
		...
		// 分析PATHINFO信息   把路径转为PATHINFO  重写什么的转成$_SERVER['PATH_INFO']
		...
		if(!empty($_SERVER['PATH_INFO'])) { //如 /model/action/id/1/page/2   如果有PATH_INFO  分解成$_GET
			tag('path_info');
			$part =  pathinfo($_SERVER['PATH_INFO']);//Array([dirname] => script    [basename] => <script>    [filename] => <script>)
