...
class  Template {
	...
	public function loadTemplate ($tmplTemplateFile,$prefix='') {
		...
		//清除opcache缓冲
		if(OPCACHE_ENABLE){
			opcache_invalidate($tmplCacheFile);
		}
	...
	protected function compiler($tmplContent) {
		...
		// 添加安全代码
		$tmplContent =  '<?php if (!defined(\'JETEE_PATH\')) exit();?>'.$tmplContent;
	...
	protected function parseExtend($content) {
		...
		if($find) {
			...
			// 替换block标签
			$content=preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is',  function ($matches) {return $this->replaceBlock($matches[1],$matches[2]);},   $content);
		}else{
			$content    = 	preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is',  function ($matches) {return stripslashes($matches[2]);},   $content);        }
		return $content;
	}
		...
		foreach ($tLib->getTags() as $name=>$val){
			...
			foreach ($tags as $tag){
				...
				if (!$closeTag){
					...
					$content=preg_replace_callback($patterns,  function($matches)use ($tagLib,$tag) {return $this->parseXmlTag($tagLib,$tag,$matches[1],'');}, $content);
				}else{
					$patterns       = '/'.$begin.$parseTag.$n1.$end.'(.*?)'.$begin.'\/'.$parseTag.'(\s*?)'.$end.'/is';
					for($i=0;$i<$level;$i++) 
						$content=preg_replace_callback($patterns,  function($matches)use ($tagLib,$tag) {return $this->parseXmlTag($tagLib,$tag,$matches[1],$matches[2]);}, $content);
				}
			}
		}
	}
	...
	public function parseXmlTag($tagLib,$tag,$attr,$content) {
		...
		$tLib       =  Jetee::instance('TagLib'.ucwords(strtolower($tagLib)));
		$parse      = '_'.$tag;
