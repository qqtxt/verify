...
class DbMysqli extends Db{
	...
	public function connect($config='',$linkNum=0) {
		if ( !isset($this->linkID[$linkNum]) || IS_CLI && !$this->linkID[$linkNum]->ping()) {//没有连接  或是 有连接重连不上
			if(IS_CLI && isset($this->linkID[$linkNum])){
				Log::write('重连库失败,关闭重连');
				$this->linkID[$linkNum]->close();
			}
	...
	public function query($str) {
	   if(APP_DEBUG) file_put_contents('b.txt', $str."\r\n",FILE_APPEND); 
	...
	public function execute($str) {
	    if(APP_DEBUG) file_put_contents('a.txt', $str."\r\n",FILE_APPEND); 
	...
	protected  function parseKey(&$key) {
		...
		elseif(strpos($key,'.') && strpos($key,'`')===false && !preg_match('/[*\s]/',$key)){//table.field 变成 `table`.`field`
			$key='`'.str_replace('.','`.`',$key).'`';
		}
		return $key;
	}
