...
/**
 * 快速文件数据读取和保存 针对简单类型数据 字符串、数组    也消除了opcache影响 一个file_get_contents  一个opcache_invalidate
 * @param string $name 缓存名称
 * @param mixed $value 缓存值
 * @param string $path 缓存路径
 * @return mixed
 */
function FF($name, $value='', $path=DATA_PATH) {
	static $_cache  = array();
...
function cookie($name, $value='', $option=null) {
	...
	$config = array(
		...
		'expire'    =>  C('COOKIE_EXPIRE'), // cookie 保存时间  0是会话
