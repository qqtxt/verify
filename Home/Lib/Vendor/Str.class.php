...
class Str{
	 ...
	 * @return string
	 */
	static function build_verify($length=6,$mode='',$session_name='verify') {
		$verify=self::randString($length,$mode);
		session($session_name,md5($verify));
		return $verify;
	}
	/**
	 * 检查验证码时检查邮箱或手机等是否更改
	 ...
	 * @return boolean
	 */
	static function check_captcha_name_change($name,$str){	
		$return=false;
		if(trim(session($name))===trim($str)){
			session($name,null);
			$return=true;
		}
		return $return;
	 ...
	 * @return boolean
	 */
	static function check_captcha($str,$verify_name='verify'){
		$return=false;
