...
class Image {
    //检验验证码
    static function chkVerify($verifyName,$verify){
        return $verify ? session($verifyName)===md5($verify) : false;
    }
    /**
     * 取得图像信息
     * @static
    ...
    static function buildString($string, $rgb=array(), $filename='', $type='png', $disturb=1, $border=true) {
	 ...
	 // 可用中文验证码
	 static function GBVerify($length=4,$mode=4,$type='png', $width=180, $height=50, $fontface='simhei.ttf', $verifyName='verify') {
        $code = Str::randString($length, $mode);
    ...
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        $randval = Str::randString($length, $mode);
